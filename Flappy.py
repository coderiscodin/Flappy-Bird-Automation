...
##FPS = 32
##CAPITAL VARIABLES = CONSTATNS USING GENERAL CONVENTIONS
WIN_WIDTH = 600
WIN_HEIGHT = 800
FLOOR = 730
...
class Bird:
    ...
    ROT_VEL = 20  #VELOCITY(SPEED) AT WHICH ROTATION WILL TAKE PLACE
    ...
    def __init__(self, x, y):
        ...
        #TICK COUNT COUNTS INPUT TAKEN(FOR EVERY TOUCH/KEY INPUT THE COUNT WILL INC OR DEC 
    ...
    def move(self):
        ...
        #SET THE UPWARD DISPLACEMENT LIMIT AS FROM EQUATION IN LINE 103 THE MORE IS THE TICK COUNT,MORE 
        #HIGHER WILL BE DISPLACEMENT 
        #LETS SAY A BIRD IS MOVING -1 UPWARDS FOR TICKCOUNT=3 AND <-2 FOR TICKCOUNT=4 and will go even 
        #higher with higher tick count so avoid exponential increase we set upper limit to it
        if displacement < 0:
            displacement -= 2
    ...
    def draw(self, win):
        ...
        :return: None
        """
        #COUNTING NUMBER OF UMAGES SHOWN
        self.img_count += 1
...
class Pipe():
    ...
    def __init__(self, x):
        ...
        USING ONLY X CO-ORDINATE BECAUSE HEIGHT OF THE PIPE(Y CO-ORDINATE) WILL BE GENERATED RANDOMLY
        :param x: int
        :return" None
        """
        self.x = x
        ...
        #THIS METHOD WILL DEFINE POSITION OF THE TOP AND BOTTOM PIPES AND THE REQUIRED GAP BETWEEN GAP
        self.set_height()
...
class Base:
    ...
    def move(self):
        ...
        #CONSIDER 2 SAME IMAGES OF BASE,AT THE BEGINNING OF THE GAME IMAGE1 WILL START AT X=0 ON X-AXIS 
        #AND IMAGE2 WILL BE AT X2(WIDTH OF IMAGE1).AS OUR BASE MOVES IMAGE1 AND IMAGE2 WILL GET DISPLACED
        #BY "VEL" UNITS TO THE LEFT SO THAT IT DOES NOT LOOK LIKE THERE ARE TWO DIFFERENT IMAGES
        self.x1 -= self.VEL
        self.x2 -= self.VEL
        if self.x1 + self.WIDTH < 0:

            #IF END POINT OF IMAGE1 IS AT X=0,IMAGE1 HAS BEEN DISPLACED BY "WIDTH" UNITS TO THE LEFT
            #THUS IMAGE2 IS ON THE SCREEN AND IMAGE1 IS OUT OF THE SCREEN,SO WE NOW MOVE IMAGE2 TO 
            #POSITION WHERE ORIGINALLY IMAGE2 WAS WHEN IMAGE1 WAS STARTING AT X=0 AS NOW IMAGE2 STARTS
            #AT X=0
...
def eval_genomes(genomes, config):
    ...
    while run and len(birds) > 0:
        ...
        for pipe in pipes:
            ...
            #REM[] AND THEN REMOVE THOSE PIPES FROM PIPES[](AS IN LINE-509)
...
if __name__ == '__main__': #THIS WILL ALWAYS BE TRUE AS WE ARE RUNNING CURRENT FILE AND NOT IMPORTING IT TO ANY OTHER FILE TO EXECUTE IT
